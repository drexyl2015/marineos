...
export default function SuperAdminDashboard({ data, onRefresh }: { data: DashboardData; onRefresh: () => void }) {
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard icon={<Users className="w-6 h-6" />} label="Total Crew" value={data.totalCrew} color="blue" />
        <StatCard icon={<Ship className="w-6 h-6" />} label="Active Crew" value={data.activeCrew} color="green" />
        <StatCard icon={<Clock className="w-6 h-6" />} label="Expiring Certs (90d)" value={data.expiringCerts} color="orange" />
        ...
        <div className="card p-6">
          ...
          <ResponsiveContainer width="100%" height={220}>
            <PieChart>
              <Pie
                data={[
                  { name: 'Available', value: data.availableCrew },
                  { name: 'Onboard', value: data.activeCrew },
                  { name: 'Leave', value: data.crew.filter(c => c.status === 'leave').length },
                  { name: 'Inactive', value: data.crew.filter(c => c.status === 'inactive').length },
                ].filter(d => d.value > 0)}
          ...
          {data.totalCrew === 0 ? (
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                ...
                <span className="bg-green-100 text-green-800 px-3 py-1 rounded-full font-semibold">{data.availableCrew}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-gray-700">Onboard</span>
                <span className="bg-blue-100 text-blue-800 px-3 py-1 rounded-full font-semibold">{data.activeCrew}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-gray-700">Open Alerts</span>
                <span className="bg-red-100 text-red-800 px-3 py-1 rounded-full font-semibold">{data.totalAlerts}</span>
              </div>
            </div>
          )}
        </div>
