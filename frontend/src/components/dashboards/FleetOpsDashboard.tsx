...
export default function FleetOpsDashboard({ data }: { data: any }) {
  const positions = [...new Set((data?.crew || []).map((c: any) => c.position))] as string[]
  const nationalities = [...new Set((data?.crew || []).map((c: any) => c.nationality))] as string[]

  return (
    <div className="max-w-7xl mx-auto p-4 space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard icon={<Users className="w-6 h-6" />} label="Total Fleet Crew" value={data?.totalCrew || 0} color="blue" />
        <StatCard icon={<Ship className="w-6 h-6" />} label="Deployed (Onboard)" value={data?.activeCrew || 0} color="green" />
        <StatCard icon={<UserCheck className="w-6 h-6" />} label="In Pool" value={data?.availableCrew || 0} color="purple" />
        <StatCard icon={<AlertCircle className="w-6 h-6" />} label="Open Alerts" value={data?.totalAlerts || 0} color="orange" />
      </div>
        ...
        <div className="bg-white rounded-lg shadow p-6">
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-center">
              ...
              <span className="font-bold text-green-700">{(data?.totalCerts || 0) - (data?.expiringCerts || 0)}</span>
            </div>
            <progress
            ...
            <div className="flex justify-between items-center pt-1">
              ...
              <span className="font-bold text-yellow-700">{data?.expiringCerts || 0}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-sm text-gray-700">Total Certificates</span>
              <span className="font-bold text-gray-700">{data?.totalCerts || 0}</span>
            </div>
          </div>
        </div>
      </div>
