...
export default function SeafarerDashboard({ data }: { data: any }) {
  const seafarer = data?.crew?.[0]
  const myCerts = data?.certificates?.filter((c: any) => c.crew_id === seafarer?.id) || []
  const myAlerts = data?.alerts?.filter((a: any) => a.crew_id === seafarer?.id) || []
  ...
  const nextExpiry = myCerts.length > 0
    ? myCerts.reduce((min: any, c: any) => new Date(c.expiry_date) < new Date(min.expiry_date) ? c : min)
    : null

  const daysToNext = nextExpiry
    ? Math.max(0, Math.floor((new Date(nextExpiry.expiry_date).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24)))
    : null
                ...
                <div key={i} className="flex items-center justify-between p-4 border rounded-lg hover:bg-gray-50">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      {c.issuing_authority && <p className="text-xs text-gray-400">{c.issuing_authority}</p>}
                    </div>
                  </div>
